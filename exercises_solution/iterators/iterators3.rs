...
pub fn divide(a: i32, b: i32) -> Result<i32, DivisionError> {
    match b {
		...
		_ => match (a,b) {
			(a,b) if a/b*b == a => Ok(a/b),
			(a,b) if a/b*b != a => Err(DivisionError::NotDivisible(NotDivisibleError{dividend:a,divisor:b})),
...
fn result_with_list() -> Result<Vec<i32>, DivisionError> {
	...
	//let my_result: Vec<i32> = division_results.iter().map(|x| *x.as_ref().unwrap()).collect();
	//Ok(my_result)
}
